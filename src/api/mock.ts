...
// -------------------- 响应格式 --------------------
const ok = (data: any) => ({ status: 200, data: { code: 200, data, message: 'ok' } })
const fail = (message: string, status = 400) => ({ status, data: { code: status, message } })

// -------------------- Storage Keys --------------------
const KEYS = {
    BOOKS: 'library_books', READERS: 'library_readers', BORROWS: 'library_borrows',
    SETTINGS: 'library_settings', ADMINS: 'library_admins', CATEGORIES: 'library_categories',
    ANNOUNCEMENTS: 'library_announcements', LOGS: 'library_logs', FINE_PAID: 'library_fine_paid',
}

// -------------------- 种子数据初始化 --------------------
function initAllData() {
    // ---- 图书 ----
    if (!localStorage.getItem(KEYS.BOOKS)) setData(KEYS.BOOKS, [
    ...
    // ---- 读者 ----
    if (!localStorage.getItem(KEYS.READERS)) setData(KEYS.READERS, [
    ...
    // ---- 借阅 ----
    if (!localStorage.getItem(KEYS.BORROWS)) setData(KEYS.BORROWS, [
    ...
    // ---- 分类 ----
    if (!localStorage.getItem(KEYS.CATEGORIES)) setData(KEYS.CATEGORIES, [
    ...
    // ---- 管理员 ----
    if (!localStorage.getItem(KEYS.ADMINS)) setData(KEYS.ADMINS, [
    ...
    // ---- 设置 ----
    if (!localStorage.getItem(KEYS.SETTINGS)) setObject(KEYS.SETTINGS, {
    ...
    if (!localStorage.getItem(KEYS.ANNOUNCEMENTS)) setData(KEYS.ANNOUNCEMENTS, [
...
async function handleMockRequest(config: AxiosRequestConfig): Promise<{ status: number; data: any }> {
    ...
    if (path === '/api/books/batch-delete' && method === 'POST') {
        ...
        const books = getData(KEYS.BOOKS)
        const failed: number[] = []
        ids.forEach(id => { if (borrows.some((b: any) => b.bookId === id && b.status === 0)) failed.push(id) })
        if (failed.length) {
            const titles = books.filter((b: any) => failed.includes(b.id)).map((b: any) => b.title).join('、')
            return fail(`《${titles}》尚有未归还的借阅，不能删除`)
        }
        setData(KEYS.BOOKS, books.filter((b: any) => !ids.includes(b.id))); return ok(ids.length)
    }
    ...
    if (bookMatch && method === 'PUT') {
        ...
        books[idx] = { ...books[idx], ...body }; setData(KEYS.BOOKS, books); return ok(books[idx])
    }
    if (bookMatch && method === 'DELETE') {
        ...
        books = books.filter((b: any) => b.id !== id); setData(KEYS.BOOKS, books); return ok(null)
    }
    ...
    if (path === '/api/readers' && method === 'POST') {
        ...
        readers.push(newReader); setData(KEYS.READERS, readers); return ok(newReader)
    }
    if (path === '/api/readers/import' && method === 'POST') {
        ...
        readers.push(...newReaders); setData(KEYS.READERS, readers); return ok(newReaders.length)
    }
    if (path === '/api/readers/batch-delete' && method === 'POST') {
        ...
        setData(KEYS.READERS, readers.filter((r: any) => !ids.includes(r.id))); return ok(ids.length)
    }
    ...
    if (readerMatch && method === 'PUT') {
        ...
        readers[idx] = { ...readers[idx], ...body }; setData(KEYS.READERS, readers); return ok(readers[idx])
    }
    if (readerMatch && method === 'DELETE') {
        ...
        readers = readers.filter((r: any) => r.id !== id); setData(KEYS.READERS, readers); return ok(null)
    }
    ...
    if (path === '/api/borrows' && method === 'POST') {
        ...
        borrows.push(newBorrow); setData(KEYS.BORROWS, borrows)
        book.quantity -= 1; setData(KEYS.BOOKS, books)
        return ok(newBorrow)
    }
    if (path === '/api/borrows/batch-delete' && method === 'POST') {
        ...
        borrows = borrows.filter((b: any) => !ids.includes(b.id)); setData(KEYS.BORROWS, borrows)
    ...
    if (borrowReturnMatch && method === 'POST') {
        ...
        if (book) { book.quantity += 1; setData(KEYS.BOOKS, books) }
        return ok(null)
    }
    ...
    if (path === '/api/categories' && method === 'GET') {
        ...
        categories.forEach((cat: any) => { directCount[cat.id] = books.filter((b: any) => b.category === cat.name).length })
    ...
    if (path === '/api/categories' && method === 'POST') {
        ...
        categories.push(newCat); setData(KEYS.CATEGORIES, categories); return ok(newCat)
    }
    if (path === '/api/categories/batch-delete' && method === 'POST') {
        ...
        setData(KEYS.CATEGORIES, categories.filter((c: any) => !ids.includes(c.id))); return ok(ids.length)
    }
    ...
    if (categoryMatch && method === 'PUT') {
        ...
        categories[idx] = { ...categories[idx], ...body }; setData(KEYS.CATEGORIES, categories)
    ...
    if (categoryMatch && method === 'DELETE') {
        ...
        setData(KEYS.CATEGORIES, categories.filter((c: any) => c.id !== id)); return ok(null)
    }
    ...
    if (path === '/api/admins' && method === 'POST') {
        ...
        admins.push(newAdmin); setData(KEYS.ADMINS, admins); return ok({ ...newAdmin, password: '********' })
    }
    ...
    if (adminMatch && method === 'PUT') {
        ...
        admins[idx] = { ...admins[idx], ...body }; setData(KEYS.ADMINS, admins); return ok({ ...admins[idx], password: '********' })
    }
    if (adminMatch && method === 'DELETE') {
        ...
        setData(KEYS.ADMINS, admins.filter((a: any) => a.id !== id)); return ok(null)
    }

    // ==================== 设置 ====================
    if (path === '/api/settings' && method === 'GET') return ok(getObject(KEYS.SETTINGS) || {})
    if (path === '/api/settings' && method === 'PUT') { setObject(KEYS.SETTINGS, body); return ok(body) }
    ...
    if (path === '/api/announcements' && method === 'POST') {
        ...
        list.push(item); setData(KEYS.ANNOUNCEMENTS, list); return ok(item)
    }
    if (path === '/api/announcements/batch-delete' && method === 'POST') {
        ...
        setData(KEYS.ANNOUNCEMENTS, list); return ok(ids.length)
    }
    ...
    if (announcementMatch && method === 'PUT') {
        ...
        list[idx] = { ...list[idx], ...body }; setData(KEYS.ANNOUNCEMENTS, list); return ok(list[idx])
    }
    if (announcementMatch && method === 'DELETE') {
        const id = Number(announcementMatch.id)
        let list = getData(KEYS.ANNOUNCEMENTS)
        list = list.filter((a: any) => a.id !== id)
        setData(KEYS.ANNOUNCEMENTS, list); return ok(null)
    }
    ...
    if (path === '/api/logs' && method === 'POST') {
        ...
        const newLog = { ...body, id: nextId(logs), time: new Date().toLocaleString('zh-CN', { hour12: false }) }
        logs.push(newLog); setData(KEYS.LOGS, logs); return ok(newLog)
    }
    if (path === '/api/logs' && method === 'DELETE') { setData(KEYS.LOGS, []); return ok(null) }
    ...
    if (path === '/api/fines/pay' && method === 'POST') {
        ...
        setData(KEYS.FINE_PAID, paid); return ok(null)
    }
    ...
    // 未匹配的路由
    return { status: 404, data: { code: 404, message: `接口不存在: ${method} ${path}` } }
}
